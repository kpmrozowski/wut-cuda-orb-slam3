...
KeyFrame::KeyFrame():
        ...
        mnBALocalForKF(0), mnBAFixedForKF(0), mnNumberOfOpt(0), mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0),
        mnRelocWords(0), mnMergeQuery(0), mnMergeWords(0), mnPlaceRecognitionQuery(0), mnPlaceRecognitionWords(0),
        mPlaceRecognitionScore(0), mbCurrentPlaceRecognition(false), mnBAGlobalForKF(0), mnMergeCorrectedForKF(0),
        mnBALocalForMerge(0), fx(0), fy(0), cx(0), cy(0), invfx(0), invfy(0), mbf(0), mb(0), mThDepth(0), N(0),
        mvKeys(static_cast<vector<cv::KeyPoint> >(0)), mvKeysUn(static_cast<vector<cv::KeyPoint> >(0)),
        ...
        mbHasVelocity(false), mbFirstConnection(true), mpParent(NULL), mbNotErase(false), mbToBeErased(false),
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnNumberOfOpt(0), mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0),
    mnRelocWords(0), mnPlaceRecognitionQuery(0), mnPlaceRecognitionWords(0), mPlaceRecognitionScore(0), mbCurrentPlaceRecognition(false),
    mnBAGlobalForKF(0), mnMergeCorrectedForKF(0), mnBALocalForMerge(0), fx(F.fx), fy(F.fy), cx(F.cx),
    cy(F.cy), invfx(F.invfx), invfy(F.invfy), mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), mDistCoef(F.mDistCoef), N(F.N), mvKeys(F.mvKeys),
    ...
    mPrevKF(NULL), mNextKF(NULL), mpImuPreintegrated(F.mpImuPreintegrated), mImuCalib(F.mImuCalib), mNameFile(F.mNameFile),
    mnDataset(F.mnDataset), mbHasVelocity(false), mTlr(F.GetRelativePoseTlr()), mTrl(F.GetRelativePoseTrl()),
    ...
    mpMap(pMap),mK_(F.mK_), mpCamera(F.mpCamera), mpCamera2(F.mpCamera2), mvLeftToRightMatch(F.mvLeftToRightMatch),
    mvRightToLeftMatch(F.mvRightToLeftMatch), mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright)
{
    mnId=nNextId++;
