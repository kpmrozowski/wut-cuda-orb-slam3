...
BOOST_AUTO_TEST_CASE(struct_with_array)
{
    ...
    size_t len = 100000;
    std::vector<int> expected_results(len);

    for (int i = 0; i < len; ++i) {
        structs.push_back(make_struct_with_array(i+1, i+2, i+3), queue);
        expected_results.at(i) = 3*i + 6;
        if (i == 2) ++expected_results.at(i);
    }
    ...
    compute::vector<int> results(len, context);
    compute::transform(
        structs.begin(), structs.end(), results.begin(), sum_array, queue
    );
    // BOOST_TEST(results == expected_results, boost::test_tools::per_element() );
    BOOST_CHECK_EQUAL_COLLECTIONS(results.begin(), results.end(), expected_results.begin(), expected_results.end());
}
